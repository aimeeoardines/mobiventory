...
class ModelHelper:
    ...
    # def get_staff(self, emp_id):
    #     return Staff.objects.filter(emp_id=emp_id).values()

    # def is_staff_valid(self, emp_id):
    #     staff = self.get_staff(emp_id=emp_id)
    #     if staff:
    #         staff_dict = dict(staff[0])
    #         staff_dict['is_authorized'] = 'true'
    #         return staff_dict
    #     return {'is_authorized': 'false'}

    # FUNCTION HELPERS
    # def assign_category_per_device_status(self, statuses):
    #     for status in statuses:
    #         category = Category.objects.filter(id=status['category_id']).values()[0]
    #         status.update(category)
    #     return statuses

    # def assign_device_per_status(self, statuses):
    #     for status in statuses:
    #         try:
    #             device = Device.objects.filter(id=status['device_id']).values()[0]
    #         except:
    #             device = Device.objects.filter(id=status['id']).values()[0]
    #         status.update(device)
    #     return statuses

    # def assign_device_primary_info(self, devices):
    #     self.assign_device_per_status(devices)
    #     self.assign_category_per_device_status(devices)
    #     self.assign_location_per_device(devices)
    #     return devices

    # def assign_latest_transaction_per_device(self, devices):
    #     for device in devices:
    #         device_obj = Device.objects.get(pk=device['id'])
    #         latest_transaction = Transaction.objects.filter().order_by('-transaction_date').values()[0]
    #         # device.update(latest_transaction)

    # def assign_location_per_device(self, devices):
    #     for device in devices:
    #         location = Location.objects.filter(id=device['location_id']).values()[0]
    #         device.update(location)
    #     return devices

    # def set_transaction_type_for_borrowing(self):
    #     TransactionType.objects.update_or_create(transaction_type=1)

    # def set_transaction_type_for_returning(self):
    #     TransactionType.objects.update_or_create(transaction_type=2)
